...
class Temporal_Attention(nn.Module):
    def __init__(self, dim, out_dim, heads = 3, dropout = 0.1,kernel_size=1,stride=1,num_point =17,mask=None,group_mask=None):
        ...
        self.data_bn = nn.BatchNorm1d(self.dim * self.num_point)
        self.dropout = nn.Dropout(0.25)

        self.qkv_conv = nn.Conv2d(self.dim, 2 * self.dim + self.dim, kernel_size=(1, stride),
                                      stride=(1, stride),
                                      padding=(0, int((1 - 1) / 2)))
        ...
        self.key_rel = nn.Parameter(torch.randn((2 * frames - 1, self.dim // self.heads), requires_grad=True))

        self.nn1 = nn.Linear(self.dim, out_dim)
        torch.nn.init.xavier_uniform_(self.nn1.weight)
    ...
    def forward(self,x,T = 1):
        BT, V, C = x.shape  # Input: (B*T, V, C), e.g., (2400, 17, 48)
        device = x.device
        N = BT // T  # Batch size
        #print(T)

        # Dynamically initialize key_rel based on T
        #self.key_rel = nn.Parameter(torch.randn((2 * T - 1, self.dim // self.heads), requires_grad=True).to(device))

        # Reshape to (N, C, T, V) for convolution
        x = x.view(N, T, V, C).permute(0, 3, 1, 2)  # (N, C, T, V), e.g., (24, 48, 100, 17)
        ...
        attn_out = torch.reshape(attn_out, (B, self.heads, 1, T, self.dim // self.heads))
        ...
        attn_out = attn_out.permute(0, 2, 3, 1).contiguous()  # from (B, C, T, V) → (B, T, V, C)
    ...
    def compute_flat_qkv(self, x, dk, dv, Nh):
        ...
        flat_v = torch.reshape(v, (N, Nh, dv // self.heads, V1 * T1))
        ...
        if dim == mlp_dim:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Residual(Temporal_Attention(dim, mlp_dim, heads = heads, dropout = dropout, num_point=num_point, mask=mask,group_mask=group_mask)),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout = dropout)))
                ]))
        else:
            for _ in range(depth):
                self.layers.append(nn.ModuleList([
                    Temporal_Attention(dim, mlp_dim, heads = heads, dropout = dropout, num_point=num_point, mask=mask,group_mask=group_mask),
                    Residual(LayerNormalize(mlp_dim, MLP_Block(mlp_dim, mlp_dim*2, dropout = dropout)))
                ]))
    def forward(self, x, mask = None,group_mask=None,T=1):
        for attention, mlp in self.layers:
            x = attention(x, T=T) # go to attention
            x = mlp(x) #go to MLP_Block
        return x


class TCN_TTRANSF_unit(nn.Module):
    def __init__(self, in_channels, out_channels,num_point, heads=3, stride=1, residual=True, dropout=0.1, mask=None, mask_grad=True, group_mask=None):
        super(TCN_TTRANSF_unit, self).__init__()
        # Pass mask to Spatial_Transformer
        self.transf1 = Temporal_Transformer(dim=in_channels, depth=3, heads=heads, mlp_dim=in_channels, dropout=dropout, num_point=num_point, mask=mask,group_mask=group_mask)
    ...
    def forward(self, x, mask=None,group_mask=None):
        ...
        if mask==None:
            tx = self.transf1(tx,T=T) #change input to x
        else:
            tx = self.transf1(tx, mask,T=T) #change input to x
...
class PoseFormer(nn.Module):
    def __init__(self, in_channels=3, num_person=12, num_point=17, num_head=6, graph=None, graph_args=dict()):
        ...
        self.l2 = TCN_TTRANSF_unit(48, 48, num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)      
        self.l3 = TCN_TTRANSF_unit(48, 48,num_point=num_point, heads=num_head, mask=self.A, mask_grad=False)
        self.l4 = TCN_TTRANSF_unit(48, 96, num_point=num_point, heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l5 = TCN_TTRANSF_unit(96, 96,num_point=num_point, heads=num_head, mask=self.A, mask_grad=True)
        self.l6 = TCN_TTRANSF_unit(96, 192, num_point=num_point,heads=num_head, stride=2, mask=self.A, mask_grad=True)
        self.l7 = TCN_TTRANSF_unit(192, 192, num_point=num_point,heads=num_head, mask=self.A, mask_grad=True)
